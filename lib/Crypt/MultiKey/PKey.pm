...
This attribute may hold any encrypted key format recognized by the constructor/loading code,
for delayed importing via the L</decrypt_private> method.  When this attribute is generated by the
...
sub _set_public {
   my ($self, $val)= @_;
   $self->_import_spki(_decode_base64($val));
   return $self;
}

sub private {
   @_ > 1? shift->_set_private(@_)
   : do {
      my $buf= secret;
      shift->_export_pkcs8($buf);
      $buf;
   };
}
sub _set_private {
   my ($self, $val)= @_;
   $self->_import_key(span($val))
      or croak "Unrecognized private key format";
...
to parse it during the constructor.
...
will write out an encrypted private key.  If a protection scheme is defined but
L</private_encrypted> is not available, this exports only the public key.
