 ...
 * cmk_bcrypt_pbkdf.c
 *
 * Implements OpenSSH/OpenBSD bcrypt_pbkdf (used by openssh-key-v1 private key KDF).
 *
 * This is *not* something OpenSSL exposes directly: it requires bcrypt's EksBlowfish
 * key schedule primitives (initstate/expandstate/expand0state/stream2word), which
 * are not available in OpenSSL's public Blowfish API.
 *
 * The code below is a compact, self-contained implementation derived from the
 * OpenBSD/OpenSSH reference algorithm (bcrypt_pbkdf.c + blowfish.c). It embeds
 * only the small subset of blowfish routines required for bcrypt_pbkdf.
 */
...
/* Standard Blowfish initial constants (hex digits of pi).
 * (P-array and S-boxes; total 18 + 4*256 words)
 *
 * These constants are public-domain in practice (standard Blowfish spec),
 * and match OpenBSD/OpenSSH’s implementation.
 * Copied from https://www.schneier.com/wp-content/uploads/2015/12/constants-2.txt
 */
static const uint32_t cmk_blf_init_P[18] = {
...
static void
cmk_blf_initstate(cmk_blf_ctx *c) {
   memcpy(c->P, cmk_blf_init_P, sizeof(cmk_blf_init_P));
   memcpy(c->S, cmk_blf_init_S, sizeof(cmk_blf_init_S));
}

/* Read 32-bit word from stream (wraps around) */
static uint32_t
...
/* Expand state with key material only (OpenBSD Blowfish_expand0state) */
static void
...
/* Expand state with salt+key (OpenBSD Blowfish_expandstate) */
static void
cmk_blf_expandstate(cmk_blf_ctx *c,
...
/* ---------- bcrypt hash primitive used by bcrypt_pbkdf ---------- */

#define CMK_BCRYPT_WORDS    8
#define CMK_BCRYPT_HASHSIZE (CMK_BCRYPT_WORDS * 4) /* 32 bytes */

static void
cmk_bcrypt_hash(const uint8_t sha2pass[64], const uint8_t sha2salt[64], uint8_t out[CMK_BCRYPT_HASHSIZE]) {
   cmk_blf_ctx state;
...
/*
 * Derive out_len bytes into `out` using OpenSSH/OpenBSD bcrypt_pbkdf.
 *
 * Returns true on success, false on failure and sets *err_out to a static string.
 */
static bool
cmk_bcrypt_pbkdf(const uint8_t *pass, size_t pass_len,
   ...
   uint8_t tmp[CMK_BCRYPT_HASHSIZE];
   ...
   *err_out = NULL;
   ...
   amt    = (out_len + stride - 1) / stride;

   for (count = 1; out_len > 0; count++) {
      /* countsalt = salt || BE32(count) */
      countsalt[salt_len + 0] = (count >> 24) & 0xFF;
      countsalt[salt_len + 1] = (count >> 16) & 0xFF;
      countsalt[salt_len + 2] = (count >>  8) & 0xFF;
