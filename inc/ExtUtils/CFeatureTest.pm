...
sub new {
   ...
   if ($self->{emit_unicode}) {
      # Alter the constants according to the IO layer of stdout.
      # This is sloppy, but unlikely to break.
      require PerlIO;
      if (grep /utf-?8/i, PerlIO::get_layers(\*main::STDOUT)) {
         utf8::decode($_)
            for $uchar_check, $uchar_x;
      } else {
         utf8::downgrade($_,1) || utf8::encode($_)
            for $uchar_check, $uchar_x;
      }
   }
   $self->{config_headers}= '';
...
sub emit_tty { $_[0]{emit_tty} }
sub emit_unicode { $_[0]{emit_unicode} }
sub cbuilder {
